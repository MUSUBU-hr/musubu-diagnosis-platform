...
【重要】analysis・weapon・environment・motivationの4項目では、アラビア数字（0〜9）を一切使用しないこと。Q番号・設問番号・スコア・点数・割合など、数字を含む表現はすべて禁止。「Q41」「4/5」「3点」「80%」のような表記は絶対に使わず、数字なしの日本語で表現すること。
...
{"analysis": "...", "weapon": "...", "environment": "...", "motivation": "...", "advisor_memo": "..."}`;
    ...
    // 数字がユーザー向け項目に含まれているか検査
    function hasNumericContent(parsed) {
      const fields = [parsed.analysis, parsed.weapon, parsed.environment, parsed.motivation];
      return fields.some(f => f && /\d/.test(f));
    }
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      if (!hasNumericContent(parsed)) break; // 問題なければ終了
      console.warn(`数字を検出。リトライ ${attempt + 1}/${MAX_RETRIES}`);
    }

    // それでも残っている場合はフォールバックとして数字を含む括弧表記を除去
    if (hasNumericContent(parsed)) {
      ['analysis', 'weapon', 'environment', 'motivation'].forEach(key => {
        if (parsed[key]) parsed[key] = parsed[key].replace(/[（(][^）)]*\d[^）)]*[）)]/g, '').replace(/Q\d+/g, '').trim();
      });
    }
