...
【重要】analysis・weapon・environment・motivationの4項目では、「Q○」「Q○○」「Q○で○点」「Q○・Q○○で○/5」のようなQ番号・設問番号・スコア数値を含む表記を一切使用しないこと。これらは内部データであり、ユーザーに見せる文章に含めてはならない。
...
{"analysis": "...", "weapon": "...", "environment": "...", "motivation": "...", "advisor_memo": "..."}`;
    ...
    // Q番号・スコア表記がユーザー向け項目に含まれているか検査
    function hasQPattern(parsed) {
      const fields = [parsed.analysis, parsed.weapon, parsed.environment, parsed.motivation];
      return fields.some(f => f && /Q\d+/.test(f));
    }
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      if (!hasQPattern(parsed)) break; // 問題なければ終了
      console.warn(`Q番号表記を検出。リトライ ${attempt + 1}/${MAX_RETRIES}`);
    }

    // それでも残っている場合はフォールバックとして正規表現で除去
    if (hasQPattern(parsed)) {
      ['analysis', 'weapon', 'environment', 'motivation'].forEach(key => {
        if (parsed[key]) parsed[key] = parsed[key].replace(/（?Q\d+[^）)）]*[）)）]?/g, '').trim();
      });
    }
